...
export default class Minimap {
  ...
  subscribeToConfig() {
    ...
    subs.add(
      ...
      atom.config.observe("minimap-next.charHeight", opts, (configCharHeight) => {
        this.configCharHeight = configCharHeight
        this.updateScrollTop()
        this.emitter.emit("did-change-config")
      }),

      atom.config.observe("minimap-next.charWidth", opts, (configCharWidth) => {
        this.configCharWidth = configCharWidth
        this.updateScrollTop()
        this.emitter.emit("did-change-config")
      }),

      atom.config.observe("minimap-next.interline", opts, (configInterline) => {
        this.configInterline = configInterline
        this.updateScrollTop()
        this.emitter.emit("did-change-config")
      }),

      atom.config.observe("minimap-next.independentMinimapScroll", opts, (independentMinimapScroll) => {
        this.independentMinimapScroll = independentMinimapScroll
        this.updateScrollTop()
      }),

      atom.config.observe("minimap-next.scrollSensitivity", opts, (scrollSensitivity) => {
        this.scrollSensitivity = scrollSensitivity
      }),

      atom.config.observe("minimap-next.redrawDelay", opts, (redrawDelay) => {
        this.redrawDelay = redrawDelay
      }),
      // cdprr is shorthand for configDevicePixelRatioRounding

      atom.config.observe("minimap-next.devicePixelRatioRounding", opts, (cdprr) => {
